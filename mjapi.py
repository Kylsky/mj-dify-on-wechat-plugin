...
class _mjApi:
    ...
    def simpleChange(self, content):
        try:
            ...
            task_id = content_arr[0]
            logger.exception(task_id)
            if content.startswith("U") or content.startswith("V"):
                buttons = self.fetchButtons(task_id)
                if buttons:
                    custom_id = ""
                    for button in buttons:
                        if button["label"] == content:
                            ...
                            break
                    url = self.baseUrl + "/mj/submit/action"
                    data = {"customId": custom_id, "state":"midjourney-proxy-admin", "taskId":task_id}
